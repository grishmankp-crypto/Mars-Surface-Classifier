...
import gradio as gr
import spaces  # ZeroGPU decorator — required on HF free tier
...
model         = None
label_encoder = None
CLASS_NAMES   = ["cracked", "gravel", "sand", "sedimentary"]
NUM_CLASSES   = 4
init_error    = None
    ...
    for layer in model.layers:
        if isinstance(layer, tf.keras.Model):
            try:
                ...
                inp = model.inputs[0]
                return tf.keras.Model(inputs=inp, outputs=[inner(inp), model(inp)])
            except Exception:
                continue
    return None
...
if model is not None:
    try:
        ...
        print("✅ Grad-CAM ready." if grad_model else "⚠️ Grad-CAM unavailable.")
...
# ─────────────────────────────────────────────────────────────────────────────
# MAIN FUNCTION — @spaces.GPU tells HF to allocate a GPU for this call only
# This is required on ZeroGPU free tier. Without it the app crashes.
# ─────────────────────────────────────────────────────────────────────────────
...
def analyze_mars_image(pil_image):
    ...
    try:
        ...
        level     = ('🔴 HIGH' if uncertainty_pct > 50 else '🟡 MEDIUM' if uncertainty_pct > 25 else '🟢 LOW')
        pred_text = (f"🪨 **Predicted:** {pred_class.upper()}\n"
                     f"📊 **Confidence:** {confidence:.1f}%\n"
                     f"🔢 **Uncertainty:** {uncertainty_pct:.1f}%")
        unc_text  = f"Shannon Entropy: {entropy:.4f}\nUncertainty: {level} ({uncertainty_pct:.1f}%)"
        anom_text = (f"⚠️ ANOMALY FLAGGED  (score: {anomaly_score:.3f})"
                     if is_anomaly else
